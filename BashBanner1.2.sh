...
# VERSÃO SIMPLIFICADA E FUNCIONAL DO SCRIPT PYTHON
bashbanner_content(){
  cat <<'PY'
#!/usr/bin/env python3
"""
bashbanner - simples utilitário para exibir banners.
"""
...
def get_banner_folder_for_dir(dirpath):
    dir_name = str(dirpath).lower()
    for key, folder in DIR_BANNER_MAP.items():
        if key in dir_name:
            return folder
    return None
...
def main():
    ...
    ap.add_argument("--test", action="store_true", help="Testar todos os banners")
    args = ap.parse_args()
    ...
    if args.test:
        print("Testando banners disponíveis...")
        for folder_name in DIR_BANNER_MAP.values():
            folder = CONF_DIR / folder_name
            if folder.exists():
                banners = list(folder.glob("*.txt"))
                if banners:
                    print(f"\n{folder_name}: {len(banners)} banner(s)")
                    for banner in banners[:2]:  # Mostrar apenas 2
                        print(f"  - {banner.name}")
        return
    ...
    if args.startup:
        ...
        if text:
            ...
            # Fallback para desktop se não houver startup
    ...
    if args.dir:
        ...
        banner_folder = get_banner_folder_for_dir(dirpath)
        
        if banner_folder:
            text = find_random_banner(CONF_DIR / banner_folder)
            if text:
                sys.stdout.write(text)
        return
...
# VERSÃO SIMPLIFICADA E FUNCIONAL DO HOOK
hook_content(){
  cat <<'SH'
# bashbanner hook - versão simples e funcional
BASHBANNER_BIN="${BASHBANNER_BIN:-$HOME/.local/bin/bashbanner}"

# Apenas para shells interativos
case "$-" in *i*) : ;; *) return ;; esac

# Função para exibir banner de startup (uma vez por sessão)
if [ -z "$BASHBANNER_STARTUP_SHOWN" ]; then
    if [ -x "$BASHBANNER_BIN" ]; then
        "$BASHBANNER_BIN" --startup 2>/dev/null
    fi
    export BASHBANNER_STARTUP_SHOWN=1
fi

# Função para verificar e exibir banners de diretórios
__bashbanner_check_dir() {
    if [ -x "$BASHBANNER_BIN" ]; then
        local current_dir="$PWD"
        local dir_name=$(basename "$current_dir" | tr '[:upper:]' '[:lower:]')
        
        # Lista de diretórios que devem exibir banners
        case "$dir_name" in
            desktop|downloads|documents|pictures|music|videos|public|templates)
                "$BASHBANNER_BIN" --dir "$current_dir" 2>/dev/null
                ;;
        esac
        
        # Verificar também pelo caminho completo (para diretórios com nomes diferentes)
        case "$current_dir" in
            "$HOME/Desktop"|"$HOME/Downloads"|"$HOME/Documents"|"$HOME/Pictures"|"$HOME/Music"|"$HOME/Videos"|"$HOME/Public"|"$HOME/Templates")
                "$BASHBANNER_BIN" --dir "$current_dir" 2>/dev/null
                ;;
        esac
    fi
    ...
    case ":${PROMPT_COMMAND:-}:" in
        ...
        *) 
            if [ -z "$PROMPT_COMMAND" ]; then
                PROMPT_COMMAND="__bashbanner_check_dir"
            else
                PROMPT_COMMAND="${PROMPT_COMMAND};__bashbanner_check_dir"
            fi
            ;;
    esac
elif [ -n "${ZSH_VERSION:-}" ]; then
    # Zsh: usar chpwd hook
    __bashbanner_zsh_chpwd() {
        __bashbanner_check_dir
    }
    autoload -U add-zsh-hook 2>/dev/null || true
    add-zsh-hook chpwd __bashbanner_zsh_chpwd 2>/dev/null || true
fi

# Comandos úteis
bashbanner-test() {
    if [ -x "$BASHBANNER_BIN" ]; then
        echo "=== Testando BashBanner ==="
        echo "1. Testando startup:"
        "$BASHBANNER_BIN" --startup
        echo -e "\n2. Testando diretórios atuais:"
        for dir in "Desktop" "Downloads" "Documents" "Pictures"; do
            if [ -d "$HOME/$dir" ]; then
                echo -e "\n--- Testando $dir ---"
                "$BASHBANNER_BIN" --dir "$HOME/$dir"
            fi
        done
        echo -e "\n=== Teste concluído ==="
    else
        echo "Erro: BashBanner não encontrado em $BASHBANNER_BIN"
    fi
}

bashbanner-list() {
    if [ -x "$BASHBANNER_BIN" ]; then
        "$BASHBANNER_BIN" --list-dirs
    fi
}
SH
}
...
do_install(){
  ...
  for sub in bannerstartup bannerdesktop bannerdownloads bannerdocuments bannerpictures bannermusic bannervideos bannerpublico bannertemplates; do
    ...
    if [ ! -d "$d" ]; then
      ...
      if [ -z "$(ls -A "$d" 2>/dev/null || true)" ]; then
        printf "╔══════════════════════════╗\n║   %-20s  ║\n║   Banner de exemplo    ║\n╚══════════════════════════╝\n\nEste é um banner de exemplo.\nAdicione seus próprios arquivos .txt aqui.\n" "$sub" > "$d/example.txt"
  ...
  echo "  1. Recarregue o shell: source ~/.bashrc"
  echo "  2. Teste: bashbanner-test"
  echo "  3. Liste pastas: bashbanner-list"
  echo ""
  echo "Após recarregar, navegue para:"
  echo "  cd ~/Documents  # Deve mostrar banner"
  echo "  cd ~/Downloads  # Deve mostrar banner"
  echo "  cd ~/Desktop    # Deve mostrar banner"
}
